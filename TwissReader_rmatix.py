...
def readTwiss(filename):
    ...
    for i in range(0,len(lines)):
        ...
        for j,_ in enumerate(cont):
            matrix[i,j] = line_content[j+1]
    return matrix
#--------------------------------------------------------------------------
#matrix = readTwiss('b0_p2.tfs')
matrix_rmatrix = readTwiss('b0_p2_rmatrix.tfs')

madx_pt = matrix[0,6]

matrix[:,2] = matrix[:,2]/madx_pt
matrix_rmatrix[:,15] = matrix_rmatrix[:,15]/madx_pt
matrix_rmatrix[:,17] = matrix_rmatrix[:,17]/madx_pt
...
plt.plot(matrix_rmatrix[0:,0]*100 , matrix_rmatrix[0:,14]*100, label='MADX2', color='c')
plt.plot(matrix_rmatrix[0:,0]*100 , matrix_rmatrix[0:,16]*100, label='MADX3', color='m')
...
plt.plot(matrix_rmatrix[0:,0]*100 , matrix_rmatrix[0:,15], label='MADX2', color='c')
plt.plot(matrix_rmatrix[0:,0]*100 , matrix_rmatrix[0:,17], label='MADX3', color='m')
...
x = matrix_rmatrix[0:,0]*100
y = matrix_rmatrix[0:,14]*100
f2 = interpolate.interp1d(x, y)
y = matrix_rmatrix[0:,16]*100
f3 = interpolate.interp1d(x, y)

MADXnew = f(fluka[:-2,2])
MADX2new = f2(fluka[:-2,2])
MADX3new = f3(fluka[:-2,2])
...
plt.plot(fluka[:-2,2] , MADX2new -fluka[:-2,0], color='c',label='Difference2')
plt.plot(fluka[:-2,2] , MADX3new -fluka[:-2,0], color='m',label='Difference3')
...
f = interpolate.interp1d(x, y)
x = matrix_rmatrix[0:,0]*100
y = matrix_rmatrix[0:,15]
f2 = interpolate.interp1d(x, y)
y = matrix_rmatrix[0:,17]
f3 = interpolate.interp1d(x, y)
...
plt.plot(fluka[:-2,2] , y2new-fluka[:-2,3],  color='c',linestyle = ':',linewidth=3.0)
plt.plot(fluka[:-2,2] , y3new-fluka[:-2,3],  color='m',linestyle = ':',linewidth=3.0)
